class Solution:
    ...
    def update(self, node, start, end, l, r, val):
        if self.lazy[node] != 0:
            self.segsq[node] += ((end - start + 1) * (self.lazy[node] * self.lazy[node]) % self.hell + 2 * self.lazy[node] * self.seg[node]) % self.hell
        ...
        if l <= start and end <= r:
            self.segsq[node] += ((end - start + 1) * (val * val) % self.hell + 2 * val * self.seg[node]) % self.hell
            self.segsq[node] %= self.hell
